...
const getAll = async () => {
  try {
    let res = await fetch(
        "http://localhost/PROYECTOS/16_Api_Rest_Productos_Front_Back/Backend_Apirest_PHP_users/users?page=1"
      ),
      json = await res.json();
    // console.log(json[0]);
    if (!res.ok)
  ...
  if (e.target === $form) {
    ...
    if (!e.target.id.value) {
      ...
      reader.onload = function () {
        fetch(
          "http://localhost/PROYECTOS/16_Api_Rest_Productos_Front_Back/Backend_Apirest_PHP_users/",
          {
            method: "POST",
            body: JSON.stringify({
      ...
      try {
        ...
        let options = {
          ...
          res = await fetch(
            `http://localhost/PROYECTOS/16_Api_Rest_Productos_Front_Back/Backend_Apirest_PHP_users/users`,
            options
          ),
          json = await res.json();
        console.log(json);
